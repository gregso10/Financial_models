...
class SimpleViewer:
    ...
    def get_score_color(self, irr: float, risk_free: float, discount: float) -> Tuple[str, str, str]:
        ...
        if irr > discount:
            return "#22c55e", "🟢", "Bon investissement" if get_language() == "fr" else "Good investment"
        elif irr > risk_free:
            return "#eab308", "🟡", "Acceptable" if get_language() == "fr" else "Acceptable"
        else:
            return "#ef4444", "🔴", "Peu rentable" if get_language() == "fr" else "Poor return"
    ...
    def display_dvf_comparison(self, city: str, price: float, surface: float):
        """Display DVF market comparison widget - SIMPLIFIED VERSION."""
        lang = get_language()
        
        dvf = DVFComparison()
        # Use the faster cached simple comparison
        ...
        with col2:
            ...
            st.metric(
                "Médiane ville" if lang == "fr" else "City median",
        ...
        st.caption(f"📊 {comparison['transaction_count']} transactions • " + get_dvf_disclaimer(lang))
    ...
    def display_profitability_alerts(self, metrics: dict, params):
        ...
        for alert in alerts:
            ...
            if alert_type == "success":
                bg_color, border_color = "#22c55e20", "#22c55e"
            elif alert_type == "warning":
                bg_color, border_color = "#eab30820", "#eab308"
            else:
                bg_color, border_color = "#ef444420", "#ef4444"
    ...
    def display_fiscal_comparison(self, params, model):
        ...
        deductible = abs(
            pnl_year1["Property Tax"].sum() + pnl_year1["Condo Fees"].sum() +
            pnl_year1["PNO Insurance"].sum() + pnl_year1["Maintenance"].sum() +
            pnl_year1["Management Fees"].sum() + pnl_year1["Loan Interest"].sum() +
        ...
        is_reel = "Réel" in rec["recommended"]
        ...
        st.markdown(
            ...
            f'<span style="color:#888;">{rec["reason"]}</span></div>',
            unsafe_allow_html=True
        )
        ...
        with col1:
            ...
            st.write(f"• " + ("Revenu imposable" if lang == "fr" else "Taxable") + f": {fmt_currency(micro.taxable_income, 0)}")
        ...
        with col2:
            ...
            st.write(f"• " + ("Revenu imposable" if lang == "fr" else "Taxable") + f": {fmt_currency(reel.taxable_income, 0)}")
            st.write(f"• " + ("Impôt total" if lang == "fr" else "Total tax") + f": {fmt_currency(reel.total_tax, 0)}")
        
        if rec["annual_savings"] > 100:
            savings_text = f"💰 Économie: {fmt_currency(rec['annual_savings'], 0)}/an" if lang == "fr" else f"💰 Savings: {fmt_currency(rec['annual_savings'], 0)}/year"
    ...
    def display(self):
        ...
        with col_input:
            with st.form("simple_form"):
                ...
                loan_rate = st.slider(
                    ...
                    min_value=1.0, max_value=6.0, value=3.5, step=0.1, format="%.1f%%"
                ) / 100
        ...
        with col_results:
            ...
            with col_m1:
                ...
                st.metric("Rendement annuel" if lang == "fr" else "Annual Return", irr_display)
            with col_m2:
                cf_display = f"{monthly_cf:+,.0f}".replace(",", " ") + " €" if lang == "fr" else f"€{monthly_cf:+,.0f}"
                st.metric("Cash-flow mensuel" if lang == "fr" else "Monthly Cash Flow", cf_display)
            with col_m3:
                st.metric(f"Gain total ({params.holding_period_years} ans)" if lang == "fr" else f"Total Gain ({params.holding_period_years} yrs)", fmt_currency(total_gain, 0))
            ...
            with col_dvf:
                self.display_dvf_comparison(results["inputs"]["location"], results["inputs"]["price"], results["inputs"]["surface_sqm"])
            ...
            with st.expander("📋 " + ("Détails du calcul" if lang == "fr" else "Calculation Details")):
                ...
                st.caption("💡 " + ("Passez en mode Expert pour affiner." if lang == "fr" else "Switch to Expert mode to refine."))